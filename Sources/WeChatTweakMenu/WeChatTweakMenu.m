...
NSString * const WeChatTweakMenuRuntimeVersion = @"2.3.7-28632+269341";

static NSString * const WCTRootMenuIdentifier = @"com.sunnyyoung.WeChatTweak.menu";

static NSString *WCTCurrentBuild(void) {
...
static BOOL WCTIsLegacyBuild(void) {
    return [WCTCurrentBuild() isEqualToString:@"28632"];
}

static NSString *WCTAdaptationTitle(void) {
    if (WCTIsLegacyBuild()) {
        return WCTText(@"适配微信 3.8.10.17（构建 28632）",
                       @"Adapted for WeChat 3.8.10.17 (build 28632)");
    }
    return WCTText(@"适配微信 4.1.12（构建 269341）",
...
- (NSView *)statusRowWithTitle:(NSString *)title detail:(NSString *)detail {
    NSTextField *icon = WCTLabel(
        @"✓",
        [NSFont boldSystemFontOfSize:20],
...
- (NSWindowController *)makeSettingsWindowController {
    ...
    NSString *build = WCTCurrentBuild();
    NSString *version =
    ...
    if (WCTRecallMarkerIsAvailable()) {
        ...
        recallTitle =
            WCTText(@"拦截撤回 Hook 未安装",
                    @"Recall Hooks Not Installed");
        recallDetail =
            WCTText(@"当前微信构建或运行架构与撤回 Hook 不匹配。",
                    @"The current WeChat build or architecture does not match the recall hooks.");
    }
    ...
    NSTextField *note = WCTLabel(
        [WCTAdaptationTitle() stringByAppendingString:
            WCTText(@"。运行时更新后需要重新启动微信。",
                    @". Restart WeChat after updating the runtime.")],
        [NSFont systemFontOfSize:11],
        NSColor.tertiaryLabelColor
    );

    NSMutableArray<NSView *> *rows = [NSMutableArray arrayWithObjects:
        title, subtitle, revoke, nil];
    if (updates != nil) {
        [rows addObject:updates];
    }
    [rows addObjectsFromArray:@[instances, launchButton, note]];
    ...
    if (updates != nil) {
        [updates.widthAnchor constraintEqualToAnchor:stack.widthAnchor].active = YES;
    }
...
- (void)showAbout:(nullable id)sender {
    ...
    NSString *capabilities = WCTRecallMarkerIsAvailable()
        ? WCTText(@"拦截撤回与头像撤回标识已启用。",
                  @"Recall prevention and avatar recall markers are enabled.")
        : WCTText(@"当前版本的撤回 Hook 未能安装。",
                  @"The recall hooks could not be installed for this build.");
